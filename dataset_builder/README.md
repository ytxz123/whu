...
2. 从 manifest 直接导出 road centerline line 数据集和 segmentation 数据集。

不再切 fixed16，不再构建 stageb，也不再输出 intersection 相关目标。
...
4. 从 Lane.geojson 读取道路中心线。
5. 按指定框大小切图，并用 review mask 过滤候选框。

### 一键生成全部数据集

现在不需要再手改 [scripts/build_dataset.py](scripts/build_dataset.py) 顶部配置，直接命令行运行即可。

如果 train 和 val 在同一个根目录下：
...
```bash
python dataset_builder/scripts/build_dataset.py --help
```

### 单步运行

如果你想分两步调试，可以分别运行下面两个脚本。
...
python dataset_builder/scripts/build_family_manifest.py \
  --dataset-root /path/to/rc_dataset \
  --output-manifest /path/to/output/dataset_test/artifacts/family_manifest.jsonl \
  --tile-size-px 512
```

再导出 line 和 segmentation 数据集：

```bash
python dataset_builder/scripts/export_patch_dataset.py \
  --family-manifest /path/to/output/dataset_test/artifacts/family_manifest.jsonl \
  --output-root /path/to/output/dataset_test
```
...
python dataset_builder/scripts/visualize_label_comparison.py \
  --family-manifest /path/to/output/dataset_test/artifacts/family_manifest.jsonl \
  --label-files /path/to/output/dataset_test/data_line/train.jsonl \
  --meta-jsonl /path/to/output/dataset_test/artifacts/meta_train.jsonl \
  --output-dir /path/to/output/dataset_test/visualize_train \
  --max-families 20 \
  --draw-keep-boxes
```
...
1. patch 原图 + 原始 Lane 标注。
2. patch 原图 + train.jsonl 里的监督线段。
3. patch 原图 + 两者叠加对比。
...
1. 训练主入口只保留各数据集根目录下的 train.jsonl 和 val.jsonl。
2. images 和 masks 按任务分别归档。
3. manifest、meta、summary 统一收进 artifacts。
...
1. user prompt 里的 box 坐标总是当前导出图像的整图范围，采用右开上界写法，比如 512x512 对应 [0,0,512,512]。
...
这些文件只用于排查和可视化，不是训练主输入。

更具体地说：

1. data_line/train.jsonl 和 data_line/val.jsonl 是 line 重建训练的主输入。
2. artifacts/meta_train.jsonl 和 artifacts/meta_val.jsonl 是和主数据一一对应的辅助定位文件。
3. 同一张大图切出来的多个 patch，要靠 meta 里的 family_id、patch_id、crop_box、source_image_path 等字段回到原始上下文。

为什么不能只用 data_line/train.jsonl 画图：

1. train.jsonl 里只有 patch 图像路径和监督线 points。
2. 它不知道这张 patch 来自哪张大图，也不知道原始 Lane.geojson 在哪里。
3. 它没有 crop_box、keep_box、family_id、source_image_path 这些字段，没法稳定回溯。
...
1. [scripts/build_dataset.py](scripts/build_dataset.py)

- 一键串联 manifest 构建和 line/segmentation 数据集导出。
- 启动前会检查 prompt 模板占位符是否合法。

2. [scripts/build_family_manifest.py](scripts/build_family_manifest.py)

- 扫描 train 和 val 样本目录。
- 读取 GeoTIFF 元信息和 review mask。
- 生成候选 crop box，并输出 family_manifest.jsonl。

3. [scripts/export_patch_dataset.py](scripts/export_patch_dataset.py)

- 读取 family manifest。
- 从 Lane.geojson 解析 road centerline。
- 裁剪图像并同时生成 line 数据集与 segmentation 数据集。

4. [scripts/visualize_label_comparison.py](scripts/visualize_label_comparison.py)

- 对比 patch 原图上的原始 Lane 标注和最终输出标签。
- 支持直接读取训练标签，也支持读取模型输出 json/jsonl。

## 参数说明

下面按 [scripts/build_dataset.py](scripts/build_dataset.py) 里的 BuildConfig 顺序说明常用参数。前面的章节优先用于日常运行，这一节主要用于查参数。
...
- 强制每个样本只处理一个固定影像相对路径。
- 适合只想验证某张 tif，而不想扫描整个 patch_tif 目录的情况。
...
- 每个样本内部原始影像目录的相对路径。
- 默认是 patch_tif。
- 如果 tif 不放在 patch_tif 下，就需要改它。

9. image_glob

- 在 image_dir_relpath 目录下扫描原始影像时使用的匹配规则。
- 默认是 *.tif。
- 如果影像是 .tiff，或者只想挑出一部分文件，可以改这个参数。
...
### visualize_label_comparison.py 常用参数
